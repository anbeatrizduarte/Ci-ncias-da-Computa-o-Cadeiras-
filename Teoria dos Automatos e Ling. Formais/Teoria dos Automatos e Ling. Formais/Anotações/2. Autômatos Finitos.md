...
> Um idioma L é aceito por um AFD se, e somente se, for aceito por um NFA

Basicamente isso nos diz que os dois principais tipos de autômatos têm o mesmo poder de reconhecimento de linguagens. Ou seja, qualquer linguagem que possa ser aceita por um AFN também pode ser aceita por ADF, e vice-versa.

Para provar isso, temos a seguinte demonstração:

1. Para provar o SE, temos:
	- Provar que todo AFN pode ser convertido em um AFD equivalente
	- Reformulando o teorema: dada uma linguagem L, se L é aceita por um AFN, então L também é aceita por um AFD

2. Para prova o SOMENTE SE, temos:
	- Esta parte é trivial, pois todo AFD é um caso especial de AFN, onde cada estado possui exatamente uma transição para cada símbolo de entrada
	- Portanto, se L é aceito por AFD, ele também será aceito por um AFN correspondente.



##### Conversão de AFN em AFD

Em um AFN, cada transição pode levar a um subconjunto de estados. Essa característica permite a conversão do AFN para um AFD.

A técnica usada para isso é chamada de <mark style="background: #D2B3FFA6;">Construção de Subconjunto</mark>.

A definição do AFN neste caso é:
- Seja $N$ = ($Q_N, Σ, δ_N, q_0, F_N$), onde
	- $Q_N$​ é o conjunto de estados do AFN
	- Σ é o alfabeto de entrada
	- $δ_N$ é a função de transição do AFN
	- $q_0$ é o estado inicial
	- $F_N$ é o conjunto de estados finais

O objetivo será construir um AFD tal que $L(D)$ = $L(N)$, ou seja, o AFD $D$ aceita a mesma linguagem que o AFN $N$.

Para realizar tal conversão, é necessário construir o AFN e sua tabela de transições. Após isso, é feita uma tabela com as concatenações dos estados da tabela de transições anterior. Logo mais, é construído o autômato determinístico.

Como exemplo, temos que uma palavra $L={w\ ∈ {a,b}∗ ∣w\ contem\ "ab"\ em\ algum\ lugar}$:

<img src= "../Imagens/Exemplo Conversão.png" width = "600">
